...
export class StacktraceService {
  ...
  getStacktrace(id:number): Promise<any> {
    return this.http.get(this.apiServerUrl.concat(`/stacktrace/${id}`), { headers: this.httpHeaders }).toPromise();
  }
  getPageStacktrace(pageSize:number = 25, pageNumber:number = 0): Promise<any> {
    return this.http.get(this.apiServerUrl.concat(`/stacktrace/?offset=${pageSize}&page=${pageNumber}`), { headers: this.httpHeaders }).toPromise();
  }
